...
class Record
{
    ...
    public function quickReport()
    {
        ...
        if (empty($mache_info)) {
            $mache_info['mache_name'] = input('macheName', '');
            $mache_info['mache_num'] = input('macheNum', '');
            $mache_info['line_num'] = input('lineNum', '');
            $mache_info['produc_num'] = input('producNum', '');
        }
        ...
        if ($model->addRecord($data)) {
            ...
            // 微信通知
            $con_id = [];
            foreach ($params['arr'] as $p) {
                $_udata = $model->getNotify('notify_con_id', 'notify_phone = ' . $p, 0, 100);
                if ($_udata) $_udata = $_udata[0];
                else continue;
