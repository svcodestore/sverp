 ...
 * @LastEditors: Mok.CH
 * @LastEditTime: 2021-07-20 10:12:56
...
class TPMCopyAliyunData extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        try {
            $host = env('ALIYUN_DB_HOST');
            $dbname = env('ALIYUN_DB_NAME');
            $dbuser = env('ALIYUN_DB_USERNAME');
            $dbpasswd = env('ALIYUN_DB_PASSWD');
            $db = new PDO('mysql:host=' . $host . ';dbname=' . $dbname, $dbuser, $dbpasswd);
        } catch (Exception $e) {
            echo $e->getMessage();
            return;
        }
